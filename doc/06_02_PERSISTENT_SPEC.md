...
永続化層は、タスクリストとセッティングおよびログイン状態を保持し、ビジネス層からのリクエストに応えてこれらの値を読み書きします。
...
DB層への読み書きアクセスは1本のQueueにより直列化され、最初のリクエスト順を守ってDBへの書き込みを行います。
DB層への読み書きAPIのリードバックを待ってから次の読み書きを行うことで、順番の追い越しを防ぎます。
...
アプリ起動ごとに、DB層からの読み込みを行い、LocalStorageを更新します。これは、他のクライアントからのアクセスを同期する目的です。同じidのタスクに関して、DB層から読み込んだ値とlocalStorageから読み込んだ値が食い違うときがあります。このときは、versionフィールドの値、およびupdatedAtフィールド（最終更新日時）の値によってDB層の値を使うか永続化層のlocalStorageの値をつかうかを切り替えます。
...
DB同期の際、DBから読み出したアイテムと、localStorageに保存されていたアイテムの間で、楽観的ロック競合が生じることがあります。原因は、2つ考えられます

1. ネットワークエラーなどによりある端末の未同期Queueに値が格納されたままアプリを終了し、その後、他の端末で同じタスクに対し編集などの操作を行った場合
2. 同時に2か所以上で本アプリを使って、同じタスクに対し編集などの操作を行った場合

本来ならば、どちらが正かをユーザーに問い合わせるところですが、VanishToDoでは簡略化のため、更新日時(updatedAtフィールド)が新しいほうを正と決めます。

DB層の値が正の場合、古いlocalStorageの値をDBの値で上書きします。localStorageが正の場合は、localStorageをアップデートせず、DB層への正の値の書き込みリクエストを未同期Queueに追加します。

**インターフェース:**
```typescript
// 詳細はtypes.tsを参照
declare function readTasks(): Promise<Result<Task[]>>;
```

**エラー:**

- ネットワークエラー: ネットワークエラーのために初期化に失敗した
  - エラー発生時点で未同期Queueはdequeueしてエラーを返す
- サーバー内部エラー: DB層の内部ロジックにバグがある
  - エラー発生時点で未同期Queueはdequeueしてエラーを返す
- 競合エラー: 1個以上のアイテムに関して、バージョンの競合が発生した
  - 競合は解消済み。そのため、そのままアプリの動作を継続可能。
...
DB書き込み時に競合をDB層から通達されることがあります。このとき、DB層から、競合を解消された結果のitemが返却されます。永続化層はそれをlocalStorageに反映させて、onError()ハンドラに更新を通知します。ビジネス層を通じて更新を通知されたプレゼンテーション層は、タスクリストを改めてlocalStorageから読み出してユーザーへの表示を更新します。
...
- ネットワークエラー: ネットワークエラーのために書き込みに失敗した
  - 未同期queueのアイテムはDeQueueされずにエラーが返ります
- サーバー内部エラー: DB層の内部ロジックにバグがある
- 競合エラー: バージョンの競合が発生した
  - 戻り値には、エラーステータスとともに競合解消後のTaskが返ります
- バリデーションエラー: DB層でのバリデーションに失敗した
  - 上記３個のエラーでは、未同期queueのアイテムはDeQueueされてエラーが返ります
...
既存itemのupdatedAt, versionフィールドを更新したアイテムを生成します。

**インターフェース:**
```typescript
// 詳細はtypes.tsを参照
declare function touchItem<T>(data: T): DBContainer<T>;
```
