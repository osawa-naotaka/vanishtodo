...
graph LR
   ログイン画面--認証-->メイン画面
   メイン画面<--メニュー操作-->全タスク画面
   メイン画面<--メニュー操作-->完了タスク画面
   完了タスク画面<--メニュー操作-->全タスク画面
   メイン画面--タスク解析-->タスク解析画面
...
| 詳細 | 1. タスク（テキスト、最大500文字）と重さ・締切を登録し表示する<br>2. 登録したタスクはリストとして表示される<br>3. 入力したタスクはどの端末からでも見ることができる<br>4. 入力後は入力欄がクリアされ次の入力ができる |
| 操作 | エンターキー or 登録ボタン |
| 対応画面 | メイン画面 |
| 優先度 | 必須 |
| 対応US | US-1.1 クイックタスク入力 |
